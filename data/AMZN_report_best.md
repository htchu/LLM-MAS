...
| -------------------------------- | ------------------- | --------------------- |
| 年化報酬率 (Annualized Return)   | 11.71%   | 0.47%      |
| 夏普比率 (Sharpe Ratio)          | 0.528     | 0.225        |
| 最大回撤 (Maximum Drawdown)      | -43.86%   | -48.25%      |
| 條件風險價值 (CVaR 95%)          | -4.22%   | -5.98%      |
| 年化波動率 (Annualized Volatility)| 28.69% | 42.79%    |
| 勝率 (Win Rate)                  | 38.89%       | N/A                   |
| 總交易次數 (Total Trades)        | 82       | 1           |
| 最終投資組合淨值 (Final Net Worth)| 1,195,759.75 USD | 1,007,643.18 USD |
| 初始資金 (Initial Balance)       | 1,000,000.00 USD | 1,000,000.00 USD |

## 交易記錄與圖表
* **詳細交易記錄:** 請參見 `AMZN_trades_best.csv` (僅適用於策略)
* **互動式交易圖表:** 請參見 `AMZN_trading_chart_best.html` (請用瀏覽器開啟，包含 Buy&Hold 比較)
...
* **停利設置:** 35% (入場價上漲 35% 時強制賣出)
* **動態倉位管理:** 基於 ATR (14日) 和風險比例 (2.50%) 計算最大倉位
* **獎勵塑形:** 包含波動性懲罰和利潤獎勵機制
